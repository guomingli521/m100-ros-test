...
int main(int argc,char **argv)
{
    	...
    	while(ros::ok())
	{
		...
		if(receive_gpio.gpio==1)
		{
		 vedio_hit.hith=125;
		 vedio_hit.hitl=100;
		 vedio_land.landh=180;
		 vedio_land.landl=156;
		}
		if(receive_gpio.gpio==0)
		{
		 vedio_hit.hith=180;
		 vedio_hit.hitl=156;
		 vedio_land.landh=125;
		 vedio_land.landl=100;
		}
		...
		if (permission == 1)
		{
			...
			if(flag==1)
			{
			mission=land_vedio(SMALL,vedio_land);
